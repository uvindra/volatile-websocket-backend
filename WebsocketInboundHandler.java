...
/*    private String getContextFromUrl(String url) {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (msg instanceof FullHttpRequest) {
            long clientId = UUID.randomUUID().getLeastSignificantBits();
            log.debug("Client connected: " + clientId);
            channel.attr(WebsocketHandler.CLIENT_ID).set(clientId);
            WebsocketHandler.clientMap.put(clientId, channel);
            log.debug(channel.remoteAddress());

            FullHttpRequest req = (FullHttpRequest) msg;
            req.headers().add("websocket.custom.header.X-clientId", String.valueOf(clientId));
            uri = req.getUri();
            ...
            long clientId = channel.attr(WebsocketHandler.CLIENT_ID).get();
            WebsocketHandler.clientMap.remove(clientId);
            ctx.fireChannelRead(msg);
